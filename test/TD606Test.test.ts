...
const setup = deployments.createFixture(async () => {
    ...
    // setup merkle root
    const allowed = [users[0].address, users[1].address, users[2].address, users[3].address, users[4].address, users[5].address];
...
describe("TD606 Test Suite v2", () => {
    ...
    it("mints", async () => {
        ...
        const proof = merkletree
            .getHexProof(hashAddress(users[0].address));
        ...
        await expect (
            users[0].TD606.mint(inputData,proof)
    ...
    it('properly updates', async () => {
        ...
        const mint = await users[0].TD606.mint(
            inputData,
            proof,
        );

        await expect(
            deployer.TD606.updateMetadataURI(
                1,
                'poopoopeepee',
            )
    ...
    it('properly updates royalty information', async () => {
        ...
        await expect(
            (await TD606.royaltyPayoutAddress(1)).toString()
        ).to.equal(users[6].address).toString();
    });
    ...
    it('burns token', async () => {
        ...
        const mint = await users[5].TD606.mint(
            inputData,
            proof,
        );
        const mint2 = await users[5].TD606.mint(
            inputData,
            proof,
        );
        const mint3 = await users[5].TD606.mint(
            inputData,
            proof,
        );
        const mint4 = await users[5].TD606.mint(
            inputData,
            proof,
        );
    ...
    it('retrieves tokenURI correctly', async () => {
        ...
        const mint = await users[2].TD606.mint(
            inputData,
            proof,
        );
        const mint2 = await users[2].TD606.mint(
            inputData,
            proof,
        );
        const mint3 = await users[2].TD606.mint(
            inputData2,
            proof,
        );
        const mint4 = await users[2].TD606.mint(
            inputData,
            proof,
        );

        await expect(
            (await deployer.TD606.tokenURI(3)).toString()
        ).to.equal(
    ...
    it('transfers a token between users', async () => {
        ...
        const mint2 = await users[4].TD606.mint(
            inputData,
            proof,
        ).then((val) => {
            console.log('VAL', val);
        ...
        await expect(
             (await TD606.ownerOf(2)).toString()
        ).to.equal(
            users[3].address,
        )
    });

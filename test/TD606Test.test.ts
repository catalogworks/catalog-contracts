...
describe("TD606 Test Suite v2", () => {
  ...
  it("burns token", async () => {
    ...
    await expect(TD606.tokenURI(3)).to.be.revertedWith("ERC721Metadata: URI query for nonexistent token");
  });
  ...
  it("retrieves tokenURI correctly", async () => {
    ...
    await expect((await deployer.TD606.tokenURI(3)).toString()).to.equal("https://catalog.works/metadata/peepeepoopoo");
  });
  ...
  it("transfers a token between users", async () => {
    ...
    const mint2 = await users[4].TD606.mint(inputData, proof).then(val => {
      console.log("VAL", val);
    });

    // transfer token
    await users[4].TD606.transferFrom(users[4].address, users[3].address, 2);

    await expect((await TD606.ownerOf(2)).toString()).to.equal(users[3].address);
  });
  ...
  it("returns the correct token contentURI", async () => {
    ...
    await expect((await TD606.tokenContentURI(1)).toString()).to.equal("http://catalog.works/content/uri/pee");
  });
  ...
  it("returns the correct creator address of the token", async () => {
    ...
    await expect((await TD606.creator(1)).toString()).to.equal(users[3].address);
  });
  ...
  it("can have contentURI updated by admin", async () => {
    ...
    await expect((await TD606.tokenContentURI(1)).toString()).to.equal("http://catalog.works/content/uri");

    // update contentURI
    await deployer.TD606.updateContentURI(1, "poopinandpeeinyeahyeah");

    await expect((await TD606.tokenContentURI(1)).toString()).to.equal("poopinandpeeinyeahyeah");
  });
  ...
  it("returns correct royalty for EIP2981", async () => {
    ...
    await expect((await TD606.royaltyInfo(1, 10)).toString()).to.equal([users[3].address, 8].toString());
  });
});

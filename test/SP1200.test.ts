...
describe('SP1200 Test Suite v1', () => {
  ...
  it('transfers a token between users', async () => {
    ...
    const mint2 = await users[4].SP1200.mint(inputData, proof).then((val) => {
      console.log('VAL', val);
    });

    // transfer token
    await users[4].SP1200.transferFrom(users[4].address, users[3].address, 2);
  ...
  it('returns the correct token contentURI', async () => {
    ...
    // transfer token
    await users[4].SP1200.transferFrom(users[4].address, users[3].address, 1);

...
async function getInput(): Promise<string> {
  ...
  const address = await prompt(
    '\x1b[33m Input Address (to add to tree, and generate proof): \x1b[0m'
  );
...
async function main(inputAddress?: string) {
  console.group(
    '\n \n \x1b[34m',
    '༼ つ ◕_◕ ༽つ the tree utility ༼ つ ◕_◕ ༽つ',
    '\x1b[0m \n \n'
  );

  let address: string =
    inputAddress || '0x8a5847fd0e592B058c026C5fDc322AEE834B87F5';
  ...
  console.log(
    '\x1b[33m%s\x1b[0m',
    'proof for address:  (copy the red for etherscan, green for tenderly)'
  );
  console.log(
    '\x1b[36m%s\x1b[0m',
    `${address}`,

...
const fetchGraphQL = async (
    ...
    const config: AxiosRequestConfig = {
        url:
            process.env.HASURA_ENDPOINT ||
            'https://catalog.hasura.app/v1/graphql',
        method: 'POST',
        headers: {
...
const fetchMyQuery = async () => {
    console.group(
        '\n \n \x1b[36m',
        '(▀̿Ĺ̯▀̿ ̿) the artist address utility (▀̿Ĺ̯▀̿ ̿)',
        '\x1b[0m \n \n'
    );
    ...
    // format
    const formatted = result.data.catalog_users.map((artist: any) => {
        return artist.id;
    });

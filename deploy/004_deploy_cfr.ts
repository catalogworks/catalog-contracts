...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
    ...
    // Proxy deploy for OZ
    const deployCFR = await deploy('CFR', {
        from: deployer,
        contract: 'CFR',
        proxy: {
            // upgradeIndex: 0,
    ...
    if (deployCFR && deployCFR.newlyDeployed) {
        log(
            '\x1b[36m%s\x1b[0m',
            `
            contract: CFR deployed at ${deployCFR.address} 
            using ${deployCFR.receipt?.gasUsed} gas. 
            Owner (to): ${deployCFR.receipt?.to}
            implementation: ${deployCFR.implementation}
            Signed from    : ${deployCFR.receipt?.from}
            `
        );
    }
